...
export default {
  ...
  methods: {
    ...
    async fetchOptions() {
      ...
      await _this.request(
        _this.route$(_this.endPoint.route, _this.endPoint.params),
        { params: _this.endPoint.params },
        function({ data }) {
          if (data.data.data != undefined) _this.options = data.data.data;
          else {
  ...
  computed: {
    ...
    optionsData: function() {
      ...
      if (_this && _this.translated_labels)
        return _.map(options, function(option) {
          option[_this.optionName] = _this.__trans(option[_this.optionName]);
          return option;
        });
    ...
    options$() {
      if (this.endPoint) {
        ...
        return this.vuexOptions;
      }
      return this.data ? this.data : [];
    },
    optionsKey() {
      return `${
        this.endPoint ? this.endPoint.route.split(".")[0] : this.model$
      }Options`;
    },
    vuexOptions() {
    ...
    ...mapState({
      ...
      // options$$: state => state.setting[`${this.endPoint.route.split(".")[0]}Options`],
    })
  },
  watch: {
    // setting: {
